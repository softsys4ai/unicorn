...
def fci(dataset, independence_test_method=fisherz, alpha=0.05, depth=-1, max_path_length=-1, verbose=False, background_knowledge=None, cache_variables_map=None, colmap=None):
    '''
    ...
    def get_required_background(edges, lm, path):
        ...
        for i in range(path):
            imp = str(edges[i]).split(" ")[-1]
            fci_edges.append(imp + " --> "+"X"+str(lm))
        
        
        return fci_edges
    waiting_to_deleted_edges = []
    #for key, val in colmap.items(): lm = int(key)
    ...
    #fci_edges = get_required_background(edges, lm, path)
    return graph, edges
